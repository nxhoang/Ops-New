    ...
    public class LineAllocationController : Controller
    {
        ...
        public async Task<JsonResult> BulkInsertAttEmp(string factory, string[] selectedTeams, string attDate, bool isPresent)
        {
            try
            {
                ...
                if (afterwardAttendEmps != null && afterwardAttendEmps.Count > 0)
                {
                    return Json(new { IsSuccess = true, Result = afterwardAttendEmps }, JsonRequestBehavior.AllowGet);
                }
