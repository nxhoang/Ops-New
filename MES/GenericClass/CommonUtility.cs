...
namespace MES.GenericClass
{
    public class CommonUtility
    {
        ...
        /// By Tai Le (Thomas)
        /// </summary>
        /// <param name="pTextDateRange"></param>
        /// <param name="pSeperator"></param>
        /// <param name="DateFormat"></param>
        /// <param name="StartDate"></param>
        /// <param name="EndDate"></param>
        /// <returns></returns>
        public static bool DateRangeTextToDateTime(string pTextDateRange, string pSeperator, string DateFormat, out DateTime StartDate, out DateTime EndDate) {
            StartDate = DateTime.MinValue;
            EndDate = StartDate; 

            try {
                if (String.IsNullOrEmpty(pSeperator)) pSeperator = "-";

                var arrTextDateRange = Regex.Split(pTextDateRange, pSeperator);

                if (arrTextDateRange.Length != 2)
                    return false;

                if (!String.IsNullOrEmpty(DateFormat))
                {
                    StartDate = DateTime.ParseExact(arrTextDateRange[0], DateFormat, new System.Globalization.CultureInfo(""));
                    EndDate = DateTime.ParseExact(arrTextDateRange[1], DateFormat, new System.Globalization.CultureInfo(""));
                }
                else
                {
                    StartDate = DateTime.Parse(arrTextDateRange[0]);
                    EndDate = DateTime.Parse(arrTextDateRange[1]);
                }
        ...
        public static Opmt CountOperationPlan(string edition, string styleCode, string styleSize, string styleColorSerial, string revNo, string opRevNo)
        {
            ...
            else
            {
                ...
                objOpmt.ManCount = (int)opmtCount.SumManCount;
        ...
        {
            var opmtResult = OpmtBus.CountOperationPlan(opmt.Edition, opmt.StyleCode, opmt.StyleSize, opmt.StyleColorSerial,
                opmt.RevNo, opmt.OpRevNo);
            opmtResult.LastUpdateTime = DateTime.Now;

            //Update operation master.
            await Task.Run(() => OpmtBus.UpdateOpmt(opmtResult));
        }
        ...
        public static string GetMenuIdByEdition(string edition)
        {
            ...
            switch (edition)
            {
                ...
                case ConstantGeneric.EditionMes:
                    menuId = ConstantGeneric.MesMenu;
                    break;
            }
