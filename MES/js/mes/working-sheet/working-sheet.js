...
function initWorkingSheetPage() {
    ...
    getListFactories(function (facList) {
        ...
        FillDataToMultipleSelect("txtFactorySync", facList, "Factory", "Name");
        FillDataToDropDownlist("Factory", facList, "Factory", "Name");
    });

    $("#Factory").val($("#hdFactoryUser").val()).trigger('change');
}

function getListFactories(callBack) {
    //var config = ObjectConfigAjaxPost("/FactorySortingParameter/GetFactoryList", false, JSON.stringify({ }));
    var config = ObjectConfigAjaxPost("/CommonAPI/GetFactoryList", false, JSON.stringify({ }));
