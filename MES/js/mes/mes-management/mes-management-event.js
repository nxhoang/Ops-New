...
function MxCheckList(mxPackage, checkListId) {
    this.MxPackage = mxPackage;
    this.CheckListId = checkListId;
    this.Confirmer = $("#hdUserId").val();
}
//Event click button check list
function eventClickButtonCheckListReadiness() {

    $("#btnUpdateProReadiness").click(function () {
        let mxPackage = getSelectedMesPackage();

        if (isEmpty(mxPackage)) return;

        let listCheckList = [];
        if ($('#chkMesOpCheck').is(':checked')) {
            listCheckList.push(new MxCheckList(mxPackage, ProductionReadiness.MESOP));
        }

        if ($('#chkBomPatterns').is(':checked')) {
            listCheckList.push(new MxCheckList(mxPackage, ProductionReadiness.BOMPattern));
        }

        if ($('#chkPpMeetingCnf').is(':checked')) {
            listCheckList.push(new MxCheckList(mxPackage, ProductionReadiness.PPMeeting));
        }

        if ($('#chkToolReadiness').is(':checked')) {
            listCheckList.push(new MxCheckList(mxPackage, ProductionReadiness.Tools));
        }

        if ($('#chkMachineReadiness').is(':checked')) {
            listCheckList.push(new MxCheckList(mxPackage, ProductionReadiness.Machines));
        }

        if ($('#chkJigRegistered').is(':checked')) {
            listCheckList.push(new MxCheckList(mxPackage, ProductionReadiness.JIG));
        }

        if ($('#chkLineSetup').is(':checked')) {
            listCheckList.push(new MxCheckList(mxPackage, ProductionReadiness.LineSetup));
        }

        //Insert list production check list
        insertListProReadinessCheckList(mxPackage, listCheckList);

    });

    //#region readiness button
...
function eventClickOnCheckBox() {
    $('#chkMesOpCheck').click(function () {
        ...
        if ($(this).is(':checked')) {
            ...
            //delProReadinessCheckList(mxPackage, ProductionReadiness.MESOP);
        }
    });

    $("#chkBomPatterns").change(function () {
        let mxPackage = getSelectedMesPackage();
        ...
        if ($(this).is(':checked')) {
            ...
            //Remove this check list
            delProReadinessCheckList(mxPackage, ProductionReadiness.BOMPattern);
        }
    });

    $("#chkPpMeetingCnf").change(function () {
        let mxPackage = getSelectedMesPackage();
        ...
        if ($(this).is(':checked')) {
            ...
            ShowConfirmYesNo(
                ...
                , function () { $("#chkPpMeetingCnf").prop('checked', false); }
            );
        } else {
            //Remove this check list
            delProReadinessCheckList(mxPackage, ProductionReadiness.PPMeeting);
        }
    });

    $('#chkToolReadiness').change(function () {
        let mxPackage = getSelectedMesPackage();
        ...
        if ($(this).is(':checked')) {
            ...
            ShowConfirmYesNo(
                ...
                , function () { $("#chkToolReadiness").prop('checked', false); }
            );
        } else {
            //Remove this check list
            delProReadinessCheckList(mxPackage, ProductionReadiness.Tools);
        }
    });

    $("#chkMachineReadiness").click(function () {
        let mxPackage = getSelectedMesPackage();
        ...
        if ($(this).is(':checked')) {
            ...
            ShowConfirmYesNo(
                ...
                , function () { $("#chkMachineReadiness").prop('checked', false); }
            );
        } else {
            //Remove this check list
            delProReadinessCheckList(mxPackage, ProductionReadiness.Machines);
        }
    });

    $("#chkJigRegistered").click(function () {
        let mxPackage = getSelectedMesPackage();
        ...
        if ($(this).is(':checked')) {
            ...
            ShowConfirmYesNo(
                ...
                , function () { $("#chkJigRegistered").prop('checked', false); }
            );
        } else {
            //Remove this check list
            delProReadinessCheckList(mxPackage, ProductionReadiness.JIG);
        }
    });

    $("#chkLineSetup").click(function () {
        let mxPackage = getSelectedMesPackage();
        ...
        if ($(this).is(':checked')) {
            ...
            ShowConfirmYesNo(
                ...
                , function () { $("#chkLineSetup").prop('checked', false); }
            );
        } else {
            //Remove this check list
            delProReadinessCheckList(mxPackage, ProductionReadiness.LineSetup);
        }
    });
}
...
function eventSaveActualMxQuantity() {
    $("#btnUpdateMxQty").click(function () {
        let mxQty = $("#txtManualQty").val();
        let mxPkg = $("#lblMxPkgMdl").text();
        let pkgGroup = $("#lblPkgGroupMdl").text();
        let seqNo = $("#lblSeqMdl").text();

        updateManualPkgQuantity(pkgGroup, seqNo, mxPkg, mxQty);
    });
}

function eventClickOnButton() {
    $("#btnSaveProLog").click(function () {
        //Get production log
        let proLog = createMesProductionLogObject();
        //Check valid data
        if (!isValidDataMesProductionLog(proLog)) return;
        //Save production log
        SaveMesProductionLog(proLog);
    });
}
