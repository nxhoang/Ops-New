...
function getDataToDisplayChart(mesPackage) {
    ...
    var config = ObjectConfigAjaxPost(
        "/MesPkgIotMachineTimeDashboard/DisplayOperationPlan",
        false,
        JSON.stringify({ mesPkg: mesPackage, date: plnStartDate })
    );

    AjaxPostCommon(config, function (lstOpe) {
        console.log(lstOpe);
        if (lstOpe && lstOpe.IsSuccess && lstOpe.Result.length > 0) {
            list = lstOpe.Result;
            loadChart(list, mesPackage, plnStartDate);
        } else {
            let mesTitle = 'Warning';
            let mesContent = 'Mes Package not has operation plan. Please select other mes package or other factory or other date.';
            ShowMessage(mesTitle, mesContent, 'warning');
        }
    });
}
...
function loadChart(list, mesPackage, plnStartDate) {
    ...
    try {
        list.forEach(function (item) {
            ...
            var output = Math.max(item.LAST_IOT_DATA, item.LAST_IOT_DATA_DGS);
            var target = 0;
            var out_put = 0;
            var complete = 0;
            if (output >= item.MXTARGET) {
                out_put = item.MXTARGET;
                complete = output - item.MXTARGET;
            } else {
                target = item.MXTARGET - output;
                out_put = output;
            }
            
            listCompleted.push(complete);
            listOutput.push(out_put);
            listMxTarget.push(target);
    ...
    obj.listOutput = listOutput;
    obj.listCompleted = listCompleted;
    obj.listOpName = listOpName;
...
function drawChart(obj) {
    ...
    var myChart = new Chart(ctx, {
        type: 'bar',
        data: {
            labels: obj.listOpName,
            datasets: [{
                label: 'Output',
                backgroundColor: "#66FF47",
                data: obj.listOutput,
            }, {
                label: 'Target',
                backgroundColor: "#22CECE",
                data: obj.listMxTarget,
            }, {
                label: 'Completed IoT',
                backgroundColor: "#E5E5E5",
                data: obj.listCompleted,
            }]
        },
        options: {
