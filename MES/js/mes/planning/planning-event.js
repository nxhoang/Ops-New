    ...
    $("#btnSearchMtopPkg").click(function () {
        ...
        if (isMESChange) {
            ShowConfirmYesNo(
                ...
                , function () {
                    //START MOD - SON) 24/Nov/2020 - check active tab to get aomtop packages
                    //searchAOMTOPProductionPackage();

                    if (_currentTab === 1) searchAOMTOPProductionPackage();
                    else loadAomtopsPackagesModule();
                    //END MOD - SON) 24/Nov/2020
                }
                , function () { }
            );
        } else {
            //START MOD - SON) 24/Nov/2020
            //searchAOMTOPProductionPackage();
            ...
            if (CREATEMESQCO) {
                ...
                for (let i = 1; i <= dayDis; i++) {
                    ...
                    while (!isWorkingDate(dayPiloStart.toString('yyyy/MM/dd'))) {
                        dayPiloStart = new DayPilot.Date(dayPiloStart).addDays(1);
                        //console.log(dayPiloStart.toString('yyyy/MM/dd'));
                    }


                    //console.log('dayPiloStart2: ' + dayPiloStart.toString('yyyy/MM/dd'));
...
function clickTabProductionpackage() {
    $("#achQcoRanking").click(function () {
        ...
        $('#divMesModuleScheduler').hide();
        $('#divMesScheduler').show();
        //END ADD - SON) 26/Nov/2020
    });
