...
function AddNewOps() {
    ...
    if (editionReg) {
        ...
        const postData = { editionReg, desOpmt, sourceOpmt, isCopyTool, isCopyOp, isCopyBomPattern: false },
            param = JSON.stringify({ editionReg, desOpmt, sourceOpmt, isCopyTool, isCopyOp, isImportFile, sourceDb: DesDataBase.OracleDb }),
            ajaxConfig = new AjaxConfig("/OpsLayout/RegisterOp", true, param);
...
//Result after add new Ops master
function ResultAddNewOps(result, postData) {
    ...
    if (result.IsSuccess === true) {
        ...
        ShowMessageOk("001", SmsFunction.Add, MessageType.Error, MessageContext.Error, MessageType.Warning, result.IsSuccess);
    }
}
