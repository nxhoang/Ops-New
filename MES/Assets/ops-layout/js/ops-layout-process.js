...
function createSummary(sumType, data, language) {
    ...
    if ($.isEmptyObject(window.CurrentOpmt)) {
        console.log("Current operation master is null or empty");
        return;
    }
    ...
    if (!isExisted) {
        ...
        const config = {
            ...
            async: true,
            postData: JSON.stringify({ opmt: window.CurrentOpmt })
        };
        switch (sumType) {
            case "sumAllMachines":
                // vithv
                ...
                AjaxPostCommon(config, (response) => {
                    ...
                    if (processSums.length !== 0 && processes.length !== 0) {
                        ...
                        for (let p of processSums) {
                            ...
                            const imgSrc = p.ImagePath === null ? "../img/no-image.png" : ToolImageSrc + p.ImagePath;
            ...
            case "showPattern":
                {
                    ...
                    if ($.isEmptyObject(window.CurrentOpmt)) {
                        console.log("Current operation master is empty");
                        return;
                    }
                    let configPat = {
                ...
                break;
            case "sumBom":
                //if (window.CurrentOpmt.Edition === "M") return;
                var configSumBom = {
                    url: "/OpsLayout/SummarizeBomByProcess", async: true,
                    postData: JSON.stringify({ opmt: window.CurrentOpmt, language: language })
                };
            ...
            case "jigSum":
                //if (window.CurrentOpmt.Edition === "M") return;
                {
                    ...
                    const configSumJigFileByModule = {
                        ...
                        postData: JSON.stringify({ opmt: window.CurrentOpmt, language: language, sumJigMode: sumJigMode })
                    };

                    AjaxPostCommon(configSumJigFileByModule, function (response) {
                        if (response.error === undefined || response.error === null) {
                            const table = createTableForJigSummary(response, sumJigMode, window.CurrentOpmt);
            ...
            default:
                {
                    let factory = "";
                    const factoryId = (window.CurrentOpmt.Factory === null || window.CurrentOpmt.Factory === undefined) ? "" : window.CurrentOpmt.Factory;
                    if (window.CurrentOpmt.Edition === "A") {
...
function changedJigSummary() {
    //const curOpmt = GetSelectedOneRowData(gridOpsTableId);
    if ($.isEmptyObject(window.CurrentOpmt)) return;
    ...
    AjaxPostCommon(configSumJigFileByModule, function (response) {
        if (response.error === undefined || response.error === null) {
            ...
            const table = sumJigMode !== "FileName" ? createTableForJigSummary(response, sumJigMode, window.CurrentOpmt)
                : createDefaultTableForJigSummary(response, window.CurrentOpmt);
