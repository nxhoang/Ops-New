...
(() => {
    ...
    document.getElementById(BtnHideOpVideoModal).addEventListener("click", () => {
        ...
        $(".modal-backdrop.fade.in").hide("slide", { direction: "right" }, 1000);

        HideOpVideoModal = true;

        setTimeout(() => {
            //$(JsVdpModal).modal('hide');
...
function UploadMainVideoToServer() {
    //$.blockUI({ message: "<h3>Processing video...</h3>" });
...
function createProcessDropdownList(data, partNo) {
...
function createRightVideoList(videoOpdt) {
    ...
    const drpProcess = createProcessDropdownList(window.MesProcesses, videoOpdt.PartNo),
