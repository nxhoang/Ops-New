...
const InputUploadVideo1 = "inputUploadVideo1",
    ...
    BarColour1 = ["#c5d6fc", "#839dbe", "#00384f", "#34425a", "#8e9dbe", "#00384f"],
...
(() => {
    ...
    document.getElementById(BtnCloseNvpModal).addEventListener("click", () => {
        ...
        if (_appElement) {
            angular.element(_appElement).scope().$apply(() => {
                angular.element(_appElement).scope().FixedModal.IsShow = !angular.element(_appElement).scope().FixedModal.IsShow;
            });
        }
    });
    ...
    _btnMinimizeNvpModal.addEventListener("click", () => {
        ...
        angular.element(_appElement).scope().$apply(() => {
            ...
            //angular.element(_appElement).scope().SplitVideoUrl = "";
            //angular.element(_appElement).scope().selectedProcessVideo = {};
        });
    });
    ...
    // Appending progress bar to body.
                ...
                $.when.apply($, reqPromises).done(() => {
                    ...
                    if (videoServerUrl) {
                        ...
                        chat.client.sendMessage = (message, count) => {
                            ...
                            document.getElementById(DivProgressBarWrapper).style.display = "block";
                            const p = count / _videoOpdts1.filter(x => x.OpSerial !== null).length * 100;
                            document.getElementById(DivProgressBar).style.width = `${p.toFixed(2)}%`;
                            document.getElementById(DivProgressBar).innerHTML = `${p.toFixed(2)}%`;
                            if (p === 100) {
                                setTimeout(() => {
                                    document.getElementById(DivProgressBarWrapper).style.display = "none";
                                }, 3000);
                            }
                        };
...
function cropVideo1() {
    if (validateStartEndTime1(true)) {
        ...
        // Assigning total second of last cropped video to default second.
        _txtCropSec1.value = totalSecond;
...
function PreviewVideo(videoSrc, data) {
    console.log("Preview video.");
    console.log(data);

    const vd = document.getElementById(VdPreviewMax);
    vd.src = videoSrc;
    vd.currentTime = convertToSecond1(data.StartTime);
    vd.ontimeupdate = () => {
        if (vd.currentTime >= convertToSecond1(data.EndTime)) vd.pause();
    };
...
function takeVideoThumbnail(data) {
    ...
    img.addEventListener("click", () => {
        console.log("Click thumbnail video.");
        _divVideoRightList1.style.display = "block";

        if (_appElement) {
            angular.element(_appElement).scope().$apply(() => {
                angular.element(_appElement).scope().SplitVideoUrl = _vidTag1.src;
                angular.element(_appElement).scope().selectedVideo = data;
            });
        }
...
async function dropCanvas(event) {
    ...
    if (_isDraggingVideo) {
        ...
        console.log("The video was dropped to a process.");
    }
}

function allowDropVideo1(event) {
    event.preventDefault();
    //console.log("Allowing dropping video to process.");
}
...
function ClearAllVideoInfo1() {
    ...
    // Clearing on angular modal.
    if (_appElement && angular.element(_appElement).scope() && angular.element(_appElement).scope().$$phase) {
        angular.element(_appElement).scope().$apply(() => {
            angular.element(_appElement).scope().SplitVideoUrl = "";
            angular.element(_appElement).scope().selectedVideo = {};
        });
    }

    // Default second is 60
    _txtCropSec1.value = 60;
}
