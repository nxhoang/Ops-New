...
function CheckRoleStyleManagement(edition, opsConfirm) {
    ...
    if ($.isEmptyObject(UserRoleStm)) {
        GetUserRoleInfoAsync(SystemIdOps, MenuIdStm, (userRoleStm) => {
            UserRoleStm = userRoleStm;
            SetRoleForControl(UpdateRole, UserRoleStm, "btnUploadImageStyle", "", edition);
            SetRoleForControl(UpdateRole, UserRoleStm, "flImageDetail", "", edition);
        });
    }
    //END MOD - SON - 2021.01.15)
}
...
const checkRoleBaseOnActiveTab = (activeTab, edition, opConfirmation) => {
    switch (activeTab) {
        case 1:
            //Check role for style detail tab
            CheckRoleStyleManagement(edition, opConfirmation);
            break;
        case 2:
            //Check role for operation plan tab (Registry operation plans and processes)
            CheckRoleOperationManagement(edition, opConfirmation);
            break;
        case 3:
            //Check role for line balancing
            CheckRoleLineBalancing(edition, opConfirmation);
            break;
        case 4:
...
function InitPage() {
    ...
    //START MOD - SON - 2021.01.15) 18/Jan/2021
    ...
    if ($.isEmptyObject(objOpsMaster)) {
        //START MOD - SON - 2021.01.15) 16.Jan/2021 - check role for operation plan tab only
        //Check user permission.
        //CheckUserRoleOps("", "");
        checkRoleBaseOnActiveTab(2, '', '');
...
function OpsMasterFunction(row) {
    ...
    checkRoleBaseOnActiveTab(2, editionRow, row.ConfirmChk);
    //START MOD - SON - 2021.01.15
...
function GetDataForOperationTab(dataRow) {
    ...
    //initial grid process detail
    if (_initialPage) {
        BindDataToJqGridOpsDetail(styleCodeRow, styleSizeRow, styleColorRow, styleRevNoRow, styleOpRevNoRow, editionRow, MapLanguageToFlag(langId));
        _initialPage = false;
    } else {
    ...
    //START MOD - SON - 2021.01.15) 16/Jan/2021 - move to function initialProcessUploadFile
...
//START ADD - SON - 2021.01.15) 16/Jan/2021
//Initial functions upload file for process
const initialProcessUploadFile = () => {
    //Load download modal jqgrid
    //BindDataToJqGridDownloadMachineFiles("", "", "", "", "", "", "", "");
    //Init file gridview.
    BindDataToJqGridMachineFile("", "", "", "", "", "", "", "", "", "");
    ...
    //console.log('initialProcessUploadFile');
}
...
function DownloadFileOpDetail(rowObject) {
    ...
    //initial gridview download file detail
    BindDataToJqGridDownloadMachineFiles(rowObject.StyleCode, rowObject.StyleSize, rowObject.StyleColorSerial, rowObject.RevNo, rowObject.OpRevNo, ZeroPad(rowObject.OpSerial, 3), rowObject.Edition, '');
