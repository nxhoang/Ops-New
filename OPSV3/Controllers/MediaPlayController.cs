...
{
    public class MediaPlayController : ApiController
    {
        #region Fields
        ...
        #region Actions
        [HttpGet]
        public HttpResponseMessage Play(string fol, string f)
        {
            ...
            string videoDir = "~/operationvideos/";

            fol = HostingEnvironment.MapPath(videoDir) + fol; //HostingEnvironment.MapPath(ConfigurationManager.AppSettings["OpsVideoProcessServerPath"]) + fol;

            FileInfo fileInfo = new FileInfo(Path.Combine(fol, f));
            if (!fileInfo.Exists)
        ...
        //[HttpGet]
        //public HttpResponseMessage Play_Backup(string fol, string f)
        //{
        //    // This can prevent some unnecessary accesses. 
        //    // These kind of file names won't be existing at all. 
        //    if (string.IsNullOrWhiteSpace(f) || AnyInvalidFileNameChars(f))
        //        throw new HttpResponseException(HttpStatusCode.NotFound);
        //    fol = ConfigurationManager.AppSettings["Shortpath"] + fol;
        //    var videoFilePath = HostingEnvironment.MapPath(fol);
        //    FileInfo fileInfo = new FileInfo(Path.Combine(videoFilePath, f));
        //    if (!fileInfo.Exists)
        //        return new HttpResponseMessage(HttpStatusCode.NotFound);
        //    long totalLength = fileInfo.Length;
        //    RangeHeaderValue rangeHeader = base.Request.Headers.Range;
