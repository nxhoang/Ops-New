...
const _stitchingOpNameId = '3486',
    TxtProcessNumber = "txtProcessNumber",
...
const generateDefiningMultiOperation = () => {
    ...
    if (!isEmpty(opName1)) {
        ...
        if (!isEmpty(opName2) || !isEmpty(opName3) || !isEmpty(opName4) || !isEmpty(opName5) || !isEmpty(dispose))
            $('#divArrow2').show();
        //Show process
...
const clearDataOnProcessDtSubModal = () => {
    ...
    $('#txtMachineCountSub').val('1');
    $('#txtWorkerSub').val('1');
    $('#txtProcessTimeSub').val('30');
    $('#txtMaxTimeSub').val('30');
    ...
    //Remarks
    $('#txtRemarksSub').val('sub process remarks');
...
const clearDataOnAddingProcessModal = () => {
    ...
    $('#txtMachineCountMain').val('1');
    $('#txtWorkerMain').val('1');
    $('#txtProcessTimeMain').val('30');
    $('#txtMaxTimeMain').val('30');
...
const reloadSubProcessDtModal = (opnSerial) => {
    ...
    if (!$.isEmptyObject(opnt)) {
        ...
        if (StatusUpdateProcess === 1) {
            ...
            if (!$.isEmptyObject(opntImage)) {
                previewImage(opntImage.FileData, '#imgPreviewSub', '#lblImageNameSub');
            } else {
                const imageHttpLink = `${ImageHttpLink}${opnt.ImageName}`;
                //previewImageFromHttpLink(opnt.ImageName, opnt.ImageLink, '#imgPreviewSub');
                previewImageFromHttpLink(opnt.ImageName, imageHttpLink, '#imgPreviewSub');
            }

            //If new video is uploaded then preview new video otherwise preview video from http link.
            if (!$.isEmptyObject(opntVideo)) {
                previewVideo(opntVideo.FileData, '#vidPreviewSub', '#videoPreviewSub', '#lblVideoNameSub');
            } else {
...
const updateMachineForProcessNameDetail = () => {
    ...
    const opnt4 = _listOpnt.find(x => x.OpnSerial === _opnSerial.Four);
    if (!$.isEmptyObject(opnt4)) {
        opnt4.MachineType = $('#drpMachineOpName4').val();
    }
    const opnt5 = _listOpnt.find(x => x.OpnSerial === _opnSerial.Five);
    if (!$.isEmptyObject(opnt5)) {
        opnt5.MachineType = $('#drpMachineOpName5').val();
    }
};

const updateOrCreateOpntObject = (opmt, opSerial, opnSerial, opNameId, machineCount, manCount, opTime, maxTime, machineType, remarks, iotType, mainProcess, stitchingLength, stitchesPerInch, imageName, videoFile, imageData, videoData) => {
    ...
    if ($.isEmptyObject(opnt)) {
        opnt = createObjectProcessNameDetail(opmt.Edition, opmt.StyleCode, opmt.StyleSize, opmt.StyleColorSerial,
            opmt.RevNo, opmt.OpRevNo, opSerial, opNameId, opnSerial, machineCount, manCount, opTime, maxTime, machineType,
            remarks, iotType, mainProcess, imageName, videoFile, stitchingLength, stitchesPerInch);
        _listOpnt.push(opnt);
    } else {
...
const getOperationIcon = (opnSerial) => {
    console.log(`Selected OpNameId: ${$(`#drpClassificationSub`).val()}`);
...
const generateProcessNameDetail = (opnSerial, txtOpnameId, hdOpnameId, rdMainProcessId, divOpNameId) => {
    ...
    //get main process
    const mainProcess = $(rdMainProcessId).is(":checked") ? '1' : '0';
    console.log(`Main process: ${mainProcess}`);

    //update opnt if it exist or create and add it to the list if it does not exist
    updateOrCreateOpntObject(opmt, opSerial, opnSerial, $(hdOpnameId).val(), machineCount, manCount, opTime, maxTime,
        machineType, remarks, iotType, mainProcess, stitchingLength, stitchesPerInch, '', '', imageData, videoData);
...
const checkDataSubProcessDetail = () => {
    ...
    if (isEmpty($('#drpClassificationSub').val()) &&
        isEmpty($('#drpSubClassSub').val()) &&
        isEmpty($('#drpSubSubSub').val())) {
...
const createObjectProcessNameDetail = (edition, styleCode, styleSize, styleColorSerial, revNo, opRevNo, opSerial, opNameId,
    opnSerial, machineCount, manCount, opTime, maxTime, machineType, remarks, iotType, mainProcess, imageName, videoFile,
    stitchingLength, stitchesPerInch) => {
    var opnt = {
        StyleCode: styleCode,
        StyleSize: styleSize,
...
const createObjOpNameDtPickUpAndDispose = (opmt, opnSerial) => {
    ...
    // If iot type is null or undefined, NaN, empty string(''), 0 or false then return empty.
    if ($.isEmptyObject(iotType)) iotType = '';
    return createObjectProcessNameDetail(
        ...
        $('#hdOpNameIdConcept').val(), opnSerial, $('#txtMachineCountConcept').val(), $('#txtWorkerConcept').val(),
        $('#txtProcessTimeConcept').val(), $('#txtMaxTimeConcept').val(), $('#drpMachineConcept').val(),
        $('#txtRemarksConcept').val(), iotType
    );
};
...
const bindDataToGridBom = (styleCode, styleSize, styleColorSerial, revNo) => {
    $("#" + Pattern_Grid_TB).jqGrid({
        ...
        colModel: [
            ...
            {
                label: arrPatternCollName.StyleColorSerial,
                name: "StyleColorSerial",
                index: "StyleColorSerial",
                align: "center",
                width: 60
            },
            {
                label: arrPatternCollName.UnitConsumption,
                name: "UnitConsumption",
                index: "UnitConsumption",
                hidden: false
            },
...
const uploadSubProcessImage = (fileData, opnt) => {
    ...
    const sysFileName = generateSysFileNameSubProcess(extFile,
        opnt.StyleCode,
        opnt.StyleSize,
        opnt.StyleColorSerial,
        opnt.RevNo,
        opnt.OpRevNo,
        ZeroPad(opnt.OpSerial, 3),
        opnt.OpnSerial,
        opnt.Edition);
...
function CreateObjectOpDetail(objOpsMaster) {
    console.log("Creating opdt object for creating new process.");
    ...
    const opSerial = document.getElementById("txtProcessNo").value;
    console.log(opSerial.value);
...
function CheckDataAddNewProcess() {
    let check = true;
    ...
    const processNo = document.getElementById(TxtProcessNumber).value;
    console.log(processNo);

    //if (isEmpty(JqTxtProcessNumber.val())) {
    //    ColorTextbox(TxtProcessNumber, "error-border");
    //    check = false;
    //} else {
    //    RemoveClass(TxtProcessNumber, "error-border");
    //}
...
function LoadObjectOpDetailModal(objOpDetail) {
    console.log("Loading opdt to the modal.");
    ...
    console.log(`objOpDetail.OpName: ${objOpDetail.OpName}`);
    //console.log(objOpDetail.OpName);
    ...
    GetOpnts(objOpDetail.Edition, objOpDetail.StyleCode, objOpDetail.StyleSize, objOpDetail.StyleColorSerial,
        objOpDetail.RevNo, objOpDetail.OpRevNo, objOpDetail.OpSerial, objOpDetail.LanguageId, (res) => {
            console.log("Get list of opnts");
            console.log(res);
            if (res && res.IsSuccess) {
                //set list opnt to global variable
                _listOpnt = res.Result.opnts;
                ImageHttpLink = res.Result.ImageHttpLink;
                ...
                _listOpnt.forEach(opnt => {
                    ...
                    switch (opnt.OpnSerial) {
                        ...
                        case _opnSerial.Two:
                            ...
                            console.log("Displaying icon in box.");
                            // Displaying sub process icon
...
const SaveNewProcess_New = async (callBackFunc, oploCallBack) => {
    if (CheckDataAddNewProcess()) {
        //Get data from local storage.
        const opMaster = GetSelectedOneRowData(gridOpsTableId);
        ...
        const objOpDetail = CreateObjectOpDetail(opMaster);
        ...
        setTimeout(async () => {
            ...
            const statusUploadVideoSupPro = await uploadListVideoOpnt(opMaster);
            ...
            const statusUploadImg = UploadImageOpdt_New(opMaster), statusUploadVideo = UploadVideoOpdt_New(opMaster);

            if (statusUploadImg !== Fail && statusUploadVideo !== Fail) {
                const lstMachine = getMachinesList(opMaster),
                    lstTool = [];
                ...
                const blAddPro = AddNewProcess_New(objOpDetail, lstMachine, lstTool, _listOpnt);

                if (blAddPro === true) {
                    if (callBackFunc === null) {
                        // Getting opdt with languages
                        console.log("Getting opdt with language.");
                        console.log(objOpDetail);

                        $.post("/OpLayout/GetOpdt", { opdt: objOpDetail }).done((res) => {
                            console.log(res);
                            if (res.IsSuccess) {
                                ...
                                const machineName = res.Result.p.MachineName ? res.Result.p.MachineName : " ",
                                    ...
                                    layoutProcess = new LayoutProcess(res.Result.p.OpSerial.toString(), `[${opNum}] ${opName}`, res.Result.p.OpTime, res.Result.p.MachineCount,
                                        machineName, res.Result.p.ManCount, opName, res.Result.p.VnOpName, res.Result.p.GbOpName, res.Result.p.MmOpName, res.Result.p.IdOpName, res.Result.p.EtOpName,
                                        res.Result.p.Codes, res.Result.p.IconNames, res.Result.p.MainProcessArr, hotSpot, opsState, opNum, remarks, res.Result.p.OpGroup, res.Result.p.MachineType,
                                        res.Result.p.ModuleId, displayColor, window.ProcessWidth, window.ProcessHeight, window.LayoutFontSize, [], true, window.LayoutPage, window.CanDelete,
                                        showButtonPlayVideo, LayoutLeftX, LayoutTopY, "emptyGroup", "", iconNameArr, res.Result.iconUrl);
                                oploCallBack(layoutProcess); // todo

                                //Reload operation master grid
                                opMaster.Edition = $("#drpOpsMasterEdition").val();
                                ReloadJqGrid2LoCal(gridOpsTableName, opMaster);
                            }
                        });
...
function UpdateProcess_New(callBackFunc, oploCallBack) {
    console.log("Updating opdt...");

    //Check data before update
    if (!CheckDataAddNewProcess()) return;
    ...
    setTimeout(async function () {
        ...
        if (statusUploadImg !== Fail && statusUploadVideo !== Fail) {
            ...
            const blUpdate = UpdateOpDetail_New(objOpDetail, lstMachine, lstTool, _listOpnt);
            if (blUpdate === false) {
